...
void PkPdReporter::begin_time_step() {
  ...
  // std::cout <<
  // Model::DATA_COLLECTOR->blood_slide_prevalence_by_location()[0]*100 << "\t";
  // std::cout << Model::DATA_COLLECTOR->number_of_positive_by_location()[0]*100
  // / static_cast<double>(Model::DATA_COLLECTOR->popsize_by_location()[0]) <<
...
void PkPdReporter::after_time_step() {
  ...
  //        Model::DATA_COLLECTOR->blood_slide_prevalence_by_location()[0]*100
  //        << "\t"; std::cout <<
  //        Model::DATA_COLLECTOR->number_of_positive_by_location()[0]*100 /
  //        (double) Model::DATA_COLLECTOR->popsize_by_location()[0] << "\t";
  //
  //        accumulator_set<double, stats<tag::median, tag::mean, tag::variance
  //        > > acc; accumulator_set<double, stats<tag::tail_quantile<right> > >
  //        acc_q(tag::tail<right>::cache_size = 10000);
  //
  //        DoubleVector today_density;
  //        //        std::cout <<
  //        Model::POPULATION->all_persons()->vPerson().size()<<std::endl; for
  //        (int i = 0; i < Model::POPULATION->all_persons()->vPerson().size();
  //        i++) {
  //            Person* person = Model::POPULATION->all_persons()->vPerson()[i];
  //            if
  //            (person->all_clonal_parasite_populations()->parasites()->size()
  //            >= 1) {
  ...
  //
  //        accumulator_set<double, stats<tag::median, tag::mean, tag::variance
  //        > > acc_PRR; accumulator_set<double, stats<tag::tail_quantile<right>
  //        > > acc_q_PRR(tag::tail<right>::cache_size = 10000); DoubleVector
  //        PRR; for (int i = 0; i < today_density.size(); i++) {
