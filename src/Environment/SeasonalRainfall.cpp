...
SeasonalRainfall* SeasonalRainfall::build(const YAML::Node &node) {
  ...
  if (settings["filename"].IsNull()) {
    throw std::invalid_argument(
        "The seasonal rainfall filename parameter is missing.");
  }
  if (settings["period"].IsNull()) {
    throw std::invalid_argument(
        "The seasonal rainfall period parameter is missing.");
  }
  ...
  if (value->adjustments.size() != value->period) {
    throw std::invalid_argument(fmt::format(
        ...
        value->adjustments.size(), value->period));
  }
...
double SeasonalRainfall::get_seasonal_factor(const date::sys_days &today,
                                             const int &location) {
