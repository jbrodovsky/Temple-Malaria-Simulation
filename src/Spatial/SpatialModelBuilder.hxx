...
class SpatialModelBuilder {
  DELETE_COPY_AND_MOVE(SpatialModelBuilder)
...
public:
  ...
  static SpatialModel* Build(const std::string &name, const YAML::Node &node) {
    ...
    if (name == "Marshall") { return new MarshallSM(node); }
    if (name == "Wesolowski") { return new WesolowskiSM(node); }
    if (name == "WesolowskiSurface") { return new WesolowskiSurfaceSM(node); }

    // Potentially deprecated movement models
    if (name == "Barabasi") {
      // TODO This model can likely be deleted from the simulation, but at this
      // time (2023-05-05) it's unclear if we
      // TODO can do so. In the mean time, make sure there is a reminder to
    ...
    if (name == "BurkinaFaso") { return new BurkinaFaso(node); }
