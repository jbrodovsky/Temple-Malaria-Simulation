...
void Random::release() const { gsl_rng_free(G_RNG); }
...
unsigned long Random::random_uniform_int(const unsigned long &from,
...
double Random::random_uniform() { return gsl_rng_uniform(G_RNG); }
...
double Random::random_beta(const double &alpha, const double &beta) {
  ...
  if (NumberHelpers::is_zero(beta)) return alpha;
...
double Random::random_gamma(const double &shape, const double &scale) {
  // if beta =0, alpha = means
  if (NumberHelpers::is_zero(scale)) return shape;
  return gsl_ran_gamma(G_RNG, shape, scale);
}

double Random::cdf_gamma_distribution(const double &x, const double &alpha,
                                      const double &beta) {
  // if beta =0, alpha = means
  if (NumberHelpers::is_zero(beta)) return 1.0;
  return gsl_cdf_gamma_P(x, alpha, beta);
}

double Random::cdf_gamma_distribution_inverse(const double &p,
                                              const double &alpha,
                                              const double &beta) {
...
void Random::random_multinomial(const std::size_t &K, const unsigned &N,
                                double p[], unsigned n[]) {
  gsl_ran_multinomial(G_RNG, K, N, p, n);
}

void Random::random_shuffle(void* base, std::size_t base_length,
...
void Random::shuffle(void* base, const std::size_t &n,
