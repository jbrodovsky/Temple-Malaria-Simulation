 ...
 * FlatArray.hxx
 *
 * This pseudo-library provides templates for a 2D and 3D data structure that
 * has been flattened into a 1D array. This is generally going to be more
 * performant than dimensional data structures but may not be as flexible.
 */
#ifndef FLATARRAY_H
#define FLATARRAY_H
...
public:
  ...
  static Flat2D<T>* flatten(std::vector<std::vector<T>> input) {
    ...
    for (std::size_t ndx = 1; ndx < result.x_size; ndx++) {
      result->y_size = (input[ndx].size() > result->y_size) ? input[ndx].size()
                                                            : result->y_size;
    }
...
public:
  ...
  // Allocate the space for an x by y by z sized space. Note that if max(x) >
  // max(y) unexpected behavior may occur.
  ...
  void set(int x, int y, int z, T value) {
    data[x + y_size * (y + x_size * z)] = value;
  }
  ...
  static Flat3D<T>* flatten(std::vector<std::vector<std::vector<T>>> input) {
    ...
    for (std::size_t ndx = 0; ndx < result->x_size; ndx++) {
      ...
      for (std::size_t ndy = 0; ndy < input[ndx].size(); ndy++) {
        result->z_size = (input[ndx][ndy].size() > result->z_size)
                             ? input[ndx][ndy].size()
                             : result->z_size;
      }
    }
    ...
    for (std::size_t ndx = 0; ndx < result->x_size; ndx++) {
      for (std::size_t ndy = 0; ndy < result->y_size; ndy++) {
        for (std::size_t ndz = 0; ndz < result->z_size; ndz++) {
          result->set(ndx, ndy, ndz,
                      input[ndx][ndy].size() > 0 ? input[ndx][ndy][ndz] : 0);
        }
      }
    }
