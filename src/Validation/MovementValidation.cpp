...
void MovementValidation::write_movement_data() {
  ...
  if (location_count > LOCATION_COUNT_CUTOFF) {
    LOG(WARNING) << "Location count of " << location_count
                 << " exceeds maximum of " << LOCATION_COUNT_CUTOFF
                 << " distances and odds not dumped.";
    return;
  }
  ...
  MatrixWriter<DoubleVector2>::write(Model::CONFIG->spatial_distance_matrix(),
                                     DISTANCES);
  ...
  for (std::size_t ndx = 0; ndx < location_count; ndx++) {
    residents_by_location[ndx] =
        Model::CONFIG->location_db()[ndx].population_size;
  }
