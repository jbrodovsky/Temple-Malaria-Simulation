...
void ReporterUtils::output_genotype_frequency1(
  ...
  for (std::size_t loc = 0; loc < number_of_locations; loc++) {
      ...
      for (std::size_t ac = 0; ac < number_of_age_classes; ac++) {
        ...
        for (auto i = 0ull; i < size; i++) {
          ...
          for (auto* parasite_population :
            ...
            // result2[g_id] += 1;
            if (individual_genotype_map.find(g_id)
    ...
    for (auto &i : result1) { i /= sum1; }

    for (auto &i : result1) { ss << i << sep; }
  }
  ss << group_sep;
  for (auto &i : result1_all) { i /= sum1_all; }
  for (auto &i : result1_all) { ss << i << sep; }
}
...
void ReporterUtils::output_genotype_frequency2(
  ...
  for (std::size_t loc = 0; loc < number_of_locations; loc++) {
    ...
    for (auto &i : result2) { i /= sum2; }
    for (auto &i : result2) { ss << i << sep; }
  }
  // output for all locations
  ss << group_sep;
  for (auto &i : result2_all) { i /= sum2_all; }
  for (auto &i : result2_all) { ss << i << sep; }
}
...
void ReporterUtils::output_genotype_frequency3(
  ...
  for (std::size_t loc = 0; loc < number_of_locations; loc++) {
    ...
    for (auto &i : result3) { i /= sum1; }

    // this is for per location
    for (auto &i : result3) { ss << i << sep; }
  }

  // this is for all locations
  ss << group_sep;
  for (auto &i : result3_all) { i /= sum1_all; }
  for (auto &i : result3_all) { ss << i << sep; }
