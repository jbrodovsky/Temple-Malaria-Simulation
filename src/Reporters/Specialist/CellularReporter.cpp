...
void CellularReporter::initialize(int job_number, const std::string &path) {
  ...
  if (Model::CONFIG->location_db().size() > 1) {
    throw std::runtime_error(
        "CellularReporter can only be used with one location configurations.");
  }

  // Prepare the aggregate logger to record the data
  el::Configurations aggregate_reporter;
  aggregate_reporter.setToDefault();
  aggregate_reporter.set(el::Level::Info, el::ConfigurationType::Format,
                         "%msg");
  aggregate_reporter.setGlobally(el::ConfigurationType::ToFile, "true");
  aggregate_reporter.setGlobally(
      el::ConfigurationType::Filename,
      fmt::format("{}_aggregate_data_{}.csv", path, job_number));
  aggregate_reporter.setGlobally(el::ConfigurationType::ToStandardOutput,
                                 "false");
  aggregate_reporter.setGlobally(el::ConfigurationType::LogFlushThreshold,
                                 "100");
  el::Loggers::reconfigureLogger("aggregate_reporter", aggregate_reporter);

  // Log the aggregate headers
  ss << "DaysElapsed" << Csv::sep << "Population" << Csv::sep << "PfPR2to10"
     << Csv::sep << "TreatmentCoverage" << Csv::sep << "InfectedIndividuals"
     << Csv::sep << "ClinicalIndividuals" << Csv::sep << "ClinicalU5"
     << Csv::sep << "ClinicalO5" << Csv::sep << "NewInfections" << Csv::sep
     << "Treatment" << Csv::sep << "NonTreatment" << Csv::sep
     << "TreatmentFailure" << Csv::sep << "ParasiteClones" << Csv::sep
     << "Theta" << Csv::sep << "580yWeighted" << Csv::sep << "508yUnweighted"
     << Csv::sep << "Plasmepsin2xCopyWeighted" << Csv::sep
     << "Plasmepsin2xCopyUnweighted" << Csv::sep << Csv::end_line;
  ...
  detail_reporter.setGlobally(
      el::ConfigurationType::Filename,
      fmt::format("{}_detailed_data_{}.csv", path, job_number));
  ...
  blood_reporter.setGlobally(
      el::ConfigurationType::Filename,
      fmt::format("{}_blood_data_{}.csv", path, job_number));
  ...
  ss << "DaysElapsed" << Csv::sep << "Individual" << Csv::sep
     << "ParasitePopulation" << Csv::sep << "C580Y" << Csv::sep << "Density"
     << Csv::sep << "Theta" << Csv::end_line;
...
void CellularReporter::monthly_report() {
  ...
  for (auto hs = 0; hs < Person::NUMBER_OF_STATE - 1; hs++) {
    for (std::size_t ac = 0; ac < index->vPerson()[0][0].size(); ac++) {
      // Iterate over all of the genotypes, there should only be one location
      // (i.e., index 0)
      auto age_class = index->vPerson()[0][hs][ac];
      for (std::size_t ndx = 0; ndx < age_class.size(); ndx++) {
        // Get the person and count them
        auto parasites =
            age_class[ndx]->all_clonal_parasite_populations()->parasites();
        population++;
        ...
        for (std::size_t infection = 0; infection < size; infection++) {
          ...
          // WARNING - This is hard-coded on the assumption that the K13
          // Propeller locus is at index 2
          if (parasite_population->genotype()->gene_expression()[2] == 1) {
            _580yCount++;
            _580yWeighted += (1 / static_cast<double>(size));
          }

          // WARNING - This is hard-coded on the assumption that the Plasmepsin
          // locus is at index 3
  ...
  ss << Model::SCHEDULER->current_time() << Csv::sep << population << Csv::sep
     ...
     << Csv::sep << Csv::end_line;
  CLOG(INFO, "aggregate_reporter") << ss.str();
  ss.str("");

  // If we are in the detailed reporting phase, run that function
  //
  // WARNING - this is hard coded for now, assuming the model starts in
  // 2007-01-01 mutations should be enabled
  //           starting in 2018-01-10, so we want ten years of mutations (i.e.,
  //           2028-01-01) for capturing detailed data
#ifdef ENABLE_GENOTYPE_REPORTER
...
#ifdef ENABLE_BLOOD_REPORTER
  // For the blood density we want the last 20 years of the model, so 2018-01-01
  // onwards
  if (dayselapsed > (11 * 365 + 3)) { blood_density_report(); }
#endif
}

// Generate a detailed report of the C580Y blood density for all infected
    ...
    for (std::size_t ac = 0; ac < index->vPerson()[0][0].size(); ac++) {
      ...
      for (std::size_t ndx = 0; ndx < age_class.size(); ndx++) {
        ...
        for (std::size_t infection = 0; infection < size; infection++) {
          ...
          auto mutation =
              !(parasite_population->genotype()->gene_expression()[2] == 0);

          // Get the current blood density in log10
          auto blood_density =
              parasite_population->get_current_parasite_density(dayselapsed);

          // Write line
          ss << dayselapsed << Csv::sep << age_class[ndx]->get_uid() << Csv::sep
             << parasite_population->get_uid() << Csv::sep << mutation
             << Csv::sep << blood_density << Csv::sep << theta << Csv::end_line;
        }
      }
    }
  }
...
// Generate a detailed report by recording the infection status of all
// individuals in the model. The expectation is that this function will be
// called by monthly_report during the appropriate time frame.
  ...
  for (auto hs = 0; hs < Person::NUMBER_OF_STATE - 1; hs++) {
    for (std::size_t ac = 0; ac < index->vPerson()[0][0].size(); ac++) {
      ...
      for (std::size_t ndx = 0; ndx < age_class.size(); ndx++) {
        ...
        // Get the person and count them, if they aren't infected zero out the
        // row
        auto parasites =
            age_class[ndx]->all_clonal_parasite_populations()->parasites();
        if (parasites->size() == 0) {
        ...
        for (std::size_t infection = 0; infection < parasites->size();
             infection++) {
          genotype_ids.push_back(
              (*parasites)[infection]->genotype()->genotype_id());
        }

        // Sort the list and update the indicators for this individual, note
        // that there should only be one occurrence of a specific genotype in
        // the individual due to clones
        genotype_ids.sort();
        for (int count = 0; count < genotypes; count++) {
