...
private:
  const std::string CHECK_EXISTENCE =
      ...
      "'therapyrecord') AS table_existence";

  const std::string INSERT_THERAPY_PREFIX =
      "INSERT INTO sim.therapyrecord (MonthlyDataId, LocationId, TherapyId, "
      "Success, Failure, Completed) VALUES ";

  // To be appended to the INSERT_THERAPY_PREFIX, note that the last character
  // must be replaced with a semicolon
