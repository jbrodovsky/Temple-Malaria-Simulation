...
private:
  const std::string SELECT_AGGREGATION =
      "SELECT aggregationlevel FROM sim.replicate WHERE ID = {} AND endtime IS "
      "NULL";

  const std::string SELECT_LOCATION =
      "SELECT l.id, l.index "
      "FROM sim.replicate r INNER JOIN sim.location l ON l.configurationid = "
      "r.configurationid "
      "WHERE r.id = {} ORDER BY INDEX";

protected:
  const std::string SELECT_MONTHLYDATAID =
      "SELECT id FROM sim.monthlydata WHERE replicateid = {} ORDER BY "
      "dayselapsed DESC LIMIT 1";
