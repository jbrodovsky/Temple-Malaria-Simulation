...
void MACTherapy::print(std::ostream &os) const {
  for (auto ndx = 0; ndx < therapy_ids_.size(); ndx++) {
    if (ndx != 0) {
      auto* previous = dynamic_cast<SCTherapy*>(
          Model::CONFIG->therapy_db().at(therapy_ids_[ndx - 1]));
      auto separation =
          start_at_days_[ndx]
          - (start_at_days_[ndx - 1] + previous->get_max_dosing_day());
      if (separation > 0) { os << "/Break(" << separation << ")/"; }
    }
