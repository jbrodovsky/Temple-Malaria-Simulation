...
double Drug::get_current_drug_concentration(int currentTime) {
  ...
  if (days <= dosing_days_) {
    if (drug_type()->id() == 0) {
      ...
      //       return  Model::RANDOM->random_normal(starting_value_,
      //       Model::CONFIG->as_iov());
    ...
    //        return starting_value_ +
    //        Model::RANDOM->random_uniform_double(-0.1, 0.1);
    return starting_value_;
  } else {
    const auto temp =
        NumberHelpers::is_zero(drug_type_->drug_half_life())
            ? -100
...
double Drug::get_mutation_probability(double currentDrugConcentration) const {
  ...
  if (currentDrugConcentration < (0.5))
    P = 2 * drug_type_->p_mutation() * drug_type_->k()
        * currentDrugConcentration;

  else if (currentDrugConcentration >= (0.5)
           && currentDrugConcentration < 1.0) {
    P = drug_type_->p_mutation()
        * (2 * (1 - drug_type_->k()) * currentDrugConcentration
           + (2 * drug_type_->k() - 1));
...
void Drug::set_number_of_dosing_days(int dosingDays) {
  ...
  end_time_ = last_update_time_
              + drug_type_->get_total_duration_of_drug_activity(dosingDays);
}

double Drug::get_parasite_killing_rate(int &genotype_id) const {
  return drug_type_->get_parasite_killing_rate_by_concentration(
      last_update_value_,
      Model::CONFIG->EC50_power_n_table()[genotype_id][drug_type_->id()]);
}
