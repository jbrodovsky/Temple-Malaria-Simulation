 ...
 * IStrategy.h
 *
 * Define the abstract base class for drug policy strategies along with
 * associated enumeration and helper map.
 */
#ifndef ISTRATEGY_H
#define ISTRATEGY_H
...
public:
  ...
  IStrategy(std::string name, const StrategyType &type)
      : id_(-1), name_{std::move(name)}, type_{type} {}
