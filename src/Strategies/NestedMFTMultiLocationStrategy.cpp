...
NestedMFTMultiLocationStrategy::NestedMFTMultiLocationStrategy()
    : IStrategy("NestedMFTMultiLocationStrategy", NestedMFTMultiLocation) {}
...
std::string NestedMFTMultiLocationStrategy::to_string() const {
  ...
  if (distribution.size() == 0) {
    ...
    for (auto i : distribution[0]) { sstm << i << ","; }
  }
  sstm << std::endl;
  ...
  if (start_distribution.size() == 0) {
    ...
    for (auto i : start_distribution[0]) { sstm << i << ","; }
  }
  sstm << std::endl;
...
void NestedMFTMultiLocationStrategy::adjust_started_time_point(
