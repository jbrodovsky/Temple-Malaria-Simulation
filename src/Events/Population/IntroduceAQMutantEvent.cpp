...
IntroduceAQMutantEvent::IntroduceAQMutantEvent(const int &location,
...
void IntroduceAQMutantEvent::execute() {
  ...
  for (std::size_t j = 0; j < Model::CONFIG->number_of_age_classes(); ++j) {
    ...
    for (auto i = 0; i < number_of_importation_cases; i++) {
      const auto index =
          Model::RANDOM->random_uniform(number_infected_individual_in_ac);
      ...
      for (auto* pp : *(p->all_clonal_parasite_populations()->parasites())) {
        ...
        // 0, 1,3,4,5,7
        const auto allele_index =
            Model::RANDOM->random_uniform(mutant_alleles.size());
        new_genotype =
            new_genotype->combine_mutation_to(1, mutant_alleles[allele_index]);
