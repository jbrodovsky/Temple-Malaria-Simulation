...
void ImportationPeriodicallyRandomEvent::execute() {
  ...
  auto date =
      static_cast<date::year_month_day>(Model::SCHEDULER->calendar_date);
  ...
  for (auto ndx = 0; ndx < infections; ndx++) {
    ...
    do {
      age_class = Model::RANDOM->random_uniform(static_cast<unsigned long>(
          pi->vPerson()[location][Person::HostStates::SUSCEPTIBLE].size()));
    } while (pi->vPerson()[location][0][age_class].empty());

    // Get the individual
    unsigned long index = Model::RANDOM->random_uniform(
  ...
  if (static_cast<unsigned int>(date.day()) == days) {
    ...
    time = (date::sys_days{nextRun}
            - date::sys_days{Model::CONFIG->starting_date()})
               .count();
  }
...
void ImportationPeriodicallyRandomEvent::infect(Person* person,
  ...
  if (log_parasite_density_ >= Model::CONFIG->parasite_density_level()
