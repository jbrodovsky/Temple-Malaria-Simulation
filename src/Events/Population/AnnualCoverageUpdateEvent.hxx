...
private:
  ...
  void execute() override {
    ...
    for (auto ndx = 0; ndx < count; ndx++) {
      tcm_db->p_treatment_less_than_5[ndx] =
          adjust(tcm_db->p_treatment_less_than_5[ndx], rate_);
      tcm_db->p_treatment_more_than_5[ndx] =
          adjust(tcm_db->p_treatment_more_than_5[ndx], rate_);
    }
    ...
    // Log on demand
    VLOG(1) << "Updated coverage by " << rate_
            << ", scheduled next for: " << time;
  }
