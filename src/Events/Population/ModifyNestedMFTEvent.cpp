...
void ModifyNestedMFTEvent::execute() {
  ...
  if (Model::TREATMENT_STRATEGY->type() == IStrategy::NestedMFTMultiLocation) {
    new_strategy = Model::CONFIG->strategy_db()[strategy_id];
    dynamic_cast<NestedMFTMultiLocationStrategy*>(Model::TREATMENT_STRATEGY)
        ->strategy_list[0] = new_strategy;
    new_strategy->adjust_started_time_point(Model::SCHEDULER->current_time());
  }

  if (Model::TREATMENT_STRATEGY->type() == IStrategy::NestedMFT) {
    new_strategy = Model::CONFIG->strategy_db()[strategy_id];
    dynamic_cast<NestedMFTStrategy*>(Model::TREATMENT_STRATEGY)
        ->strategy_list[0] = new_strategy;
    new_strategy->adjust_started_time_point(Model::SCHEDULER->current_time());
  }
  ...
  LOG(INFO) << date::year_month_day{scheduler->calendar_date}
            << " : switch first strategy in nested strategies to "
            << new_strategy->name();
}
