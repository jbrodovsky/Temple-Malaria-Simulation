...
ImportationPeriodicallyEvent::ImportationPeriodicallyEvent(
    const int &location, const int &duration, unsigned int genotype_id,
...
void ImportationPeriodicallyEvent::schedule_event(Scheduler* scheduler,
                                                  const int &location,
                                                  const int &duration,
  ...
  if (scheduler != nullptr) {
    auto* e = new ImportationPeriodicallyEvent(location, duration, genotype_id,
                                               number_of_cases, start_day);
...
void ImportationPeriodicallyEvent::execute() {
  ...
  // next day
  schedule_event(Model::SCHEDULER, location_, duration_, genotype_id_,
                 number_of_cases_, Model::SCHEDULER->current_time() + 1);
  ...
  VLOG_IF(number_of_importation_cases > 0, 2)
      << "Day: " << Model::SCHEDULER->current_time() << " - Importing "
      << number_of_importation_cases << " at location " << location_
      << " with genotype " << genotype_id_;
  for (auto i = 0; i < number_of_importation_cases; i++) {
    std::size_t ind_ac = Model::RANDOM->random_uniform(
        static_cast<unsigned long>(pi->vPerson()[location_][0].size()));
    if (pi->vPerson()[location_][0][ind_ac].empty()) { continue; }

    std::size_t index = Model::RANDOM->random_uniform(
    ...
    // //      new genotype will have 50% change of 580Y and 50% plasmepsin-2
    ...
    auto size = Model::CONFIG->parasite_density_level()
