...
public:
  ImportationPeriodicallyEvent(const int &location = -1,
                               const int &duration = -1,
                               unsigned int genotype_id = -1,
  ...
  static void schedule_event(Scheduler* scheduler, const int &location,
                             const int &duration, unsigned int genotype_id,
                             const int &number_of_cases, const int &start_day);

  std::string name() override { return "ImportationPeriodicallyEvent"; }
